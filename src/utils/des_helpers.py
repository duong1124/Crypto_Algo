...
def des_key_generation(key: str) -> list[str]:
    ...
    for round in range(16):
        ...
        RoundKeys[round] = des_permute(preRoundKey, des_compression_permutation, 48)
...
def des_sbox_substitution(key: str, sbox: list[list[list[int]]]) -> str:
    ...
    for i in range(8):
        ...
        key_substituted[i*4 + 1] = value / 8 ; value %= 8
        key_substituted[i*4 + 2] = value / 4 ; value %= 4
        key_substituted[i*4 + 3] = value / 2 ; value %= 2
        key_substituted[i*4 + 4] = value
...
def des_function(right: str, RoundKey: str) -> str:
    ...
    """
    expanded = des_permute(right, des_expansion_permutation, 48)
